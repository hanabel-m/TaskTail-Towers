...
class ViewModel: ObservableObject{
    ...
    @Published var workTimeMode: TimerMode = .initial
    ...
    func startWorkTimer(){
        ...
        workTimer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) {_ in
            if self.workTimeRemaining > 0 {
                ...
                if self.workTimeMode == .running{
                    self.workTimeMode = .paused
                }
            }
        }
        workTimeMode = .running
    }
    ...
    func advanceOneMinuteForward(){
        ...
        switch (workTimeMode, shortBreakTimerMode, longBreakTimerMode) {
